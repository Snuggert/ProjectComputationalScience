...
for edge in edges:
    myCanvas.draw_edge(edge, 5750, 6250)
...
while True:
    ...
    if t >= N:
        ...
        if use_plot:
            ...
            plot = False
            for veh in vehs:
                if(interval[0] < veh.location < interval[1] and veh.id_nr != -1
    ...
    for edge in edges:
        ...
        for vehicle in edge.vehicles:
            myCanvas.draw_vehicle(vehicle, edge, 5750, 6250)
...
if (n_fin + n_crash) > 0:
    ...
    if write_vehicle:
        ...
        for id_nr in results:
            inf = results[id_nr]
            tup = (inf.time_interval, rate_in, limit, inf.truck, inf.t_react,
                   inf.automax)
            s = ""
