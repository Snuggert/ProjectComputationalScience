...
# probabilities
delta_t = 0.80  # seconde tot nieuwe auto
p_truck = 0.02128
...
def main():
    ...
    for i in range(num_lanes):
        this_edge = Edge([[0, i * -7], [8000, i * -7]], max_speed, tick)
        this_edge.edge_nr = i
    ...
    for edge in edges:
        my_canvas.draw_edge(edge, 5750, 6250)
    ...
    while True:
            ...
            if use_plot:
                ...
                for veh in vehs:
                    if(interval[0] < veh.location < interval[1]
                       and veh.id_nr != -1 and veh.id_nr not in collision_nr):
                        plot = True
                        break
        ...
        if place_new:
            ...
            new_veh = make_new_vehicle(edges[lane], init_speed, init_location,
                                       tick, nr)
            edges[lane].add_vehicle(new_veh)
            nr += 1

        my_canvas.clear_screen((0, 0, 0))
        ...
        for edge in edges:
            ...
            for vehicle in edge.vehicles:
                my_canvas.draw_vehicle(vehicle, edge, 5750, 6250)
                ...
                if(vehicle.location >= edgesize and
                   vehicle.id_nr not in collision_nr):
                    ...
                    if vehicle.count_this:
                        ...
                        new.t_react = vehicle.t_react
                        new.truck = vehicle.truck
                        new.automax = vehicle.auto_max
    ...
    if (n_fin + n_crash) > 0:
        ...
        if write_vehicle:
            ...
            for id_nr in results:
                inf = results[id_nr]
                tup = (inf.time_interval, rate_in, limit, inf.truck,
                       inf.t_react, inf.automax)
                s = ""
...
def make_new_vehicle(this_edge, init_speed, init_location, tick, nr):
    veh_type = "car"
